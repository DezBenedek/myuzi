...
/// Instagram-like voice message bubble with deterministic waveform bars.
class VoiceWaveBubble extends StatelessWidget {
  ...
  static List<double> barsFor(String messageId, int count) {
    final seed = messageId.hashCode;
    final rng = Random(seed);
    return List.generate(count, (_) => 0.22 + rng.nextDouble() * 0.78);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final barIdle = mine ? Colors.white.withValues(alpha: 0.35) : const Color(0xFF9BB5A8);
    final barActive = mine ? Colors.white : t.colorScheme.primary;
    final bars = barsFor(messageId, 28);
                  ...
                  children: [
                    Text(
                      ...
                      style: t.textTheme.labelLarge?.copyWith(
                        color: fg,
                        fontWeight: unread && !mine ? FontWeight.w800 : FontWeight.w600,
                      ),
                    ),
...
class _WavePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < bars.length; i++) {
      ...
      final filled = progress <= 0 ? false : (i / bars.length) <= progress;
